...
// Email configuration using Gmail with proper settings
...
app.get('/api/health', (req, res) => {
  res.json({ status: 'OK', message: 'SendGrid email server is running' });
});
...
app.post('/api/payment-success', async (req, res) => {
  try {
    ...
    res.json({ 
      ...
      emailSent: false,
      paymentId: req.body.paymentIntent?.id || 'unknown',
...
app.listen(PORT, () => {
  ...
  console.log(`\n💡 Using Gmail with app password: lxle xkgr ahsy nqrh`);
  console.log(`💡 This should work for real email delivery!`);
});
